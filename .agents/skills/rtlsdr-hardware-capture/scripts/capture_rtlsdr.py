...
Exports natively to SigMF (.sigmf-data + .sigmf-meta).
"""
...
class HeadlessRtlSdrCaptureFlowgraph(gr.top_block):
    ...
    Constructs a pipeline:
      soapy.source (RTL-SDR) -> filter.freq_xlating_fir_filter_ccc (DDC) -> analog.agc2_cc -> blocks.file_sink
    ...
    def __init__(
        ...
        # 1. RTL-SDR Hardware Source (SoapySDR)
        dev = "driver=rtlsdr"
        stream_args = "bufflen=16384"
        try:
            self.rtlsdr_source = soapy.source(dev, "fc32", 1, "", stream_args, [""], [""])
        except Exception as e:
            raise RuntimeError(
                f"Failed to initialize RTL-SDR hardware via SoapySDR: {str(e)}\n"
        ...
        self.rtlsdr_source.set_gain_mode(0, use_auto_agc)
        if not use_auto_agc:
            # Set tuner gain
            try:
                self.rtlsdr_source.set_gain(0, "TUNER", float(tuner_gain))
            except Exception:
                try:
                    self.rtlsdr_source.set_gain(0, float(tuner_gain))
                except Exception:
                    pass
            # Set IF / BB gain if supported
            try:
                self.rtlsdr_source.set_gain(0, "IF", float(if_gain))
            except Exception:
                pass
...
def execute_rtlsdr_capture(
    ...
    Returns (data_path, meta_path, sample_count, decimated_rate).
    """
    ...
    decimated_rate = samp_rate // decimation
    target_rf_freq = center_freq + offset_hz

    print(f"📡 Starting {duration_sec:.1f}s RTL-SDR capture at {center_freq/1e6:.3f} MHz (RF Target: {target_rf_freq/1e6:.3f} MHz)...")
    print(f"   Parameters: SampRate={samp_rate/1e6:.2f}MSps, Gain={tuner_gain}dB ({gain_mode}), DDC Offset={offset_hz/1e3:.1f}kHz")
    ...
    metadata = {
        "global": {
            ...
            "core:sample_rate": float(decimated_rate),
            "core:version": "1.0.0",
            "core:description": f"{description} ({center_freq/1e6:.3f} MHz center, {tuner_gain} dB gain)",
    ...
    print(f"   Meta File: {meta_path} (Decimated Rate: {decimated_rate/1e3:.1f} kSps)")

    return data_path, meta_path, sample_count, decimated_rate
