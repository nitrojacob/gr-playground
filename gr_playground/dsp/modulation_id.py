...
def classify_modulation(samples):
    ...
    if is_noise:
        ...
        # Amplitude Modulation (AM): low frequency variance, elevated C20 and amp_kurtosis
        scores["AM"] += 0.85
        ...
        # High C20 and C40: BPSK (constant envelope amp_var < 0.18) vs ASK (amplitude keying amp_var >= 0.18)
        if amp_var >= 0.18:
            scores["ASK"] += 0.85
            scores["AM"] += 0.10
        else:
        ...
        # Amplitude Shift Keying (ASK / OOK): bimodal envelope (zero_ratio >= 0.35 or high c20)
        scores["ASK"] += 0.85
        scores["AM"] += 0.10
    elif c20 < 0.35 and c40 >= 0.40:
        # Digital Quadrature Modulations: QPSK (amp_var < 0.10) vs 16-QAM (0.10 <= amp_var < 0.18) vs 64-QAM (amp_var >= 0.18)
        if amp_var < 0.10:
            scores["QPSK"] += 0.85
            scores["16QAM"] += 0.30
        elif amp_var < 0.18:
